...
with open(path_method + 'VEMethod_Sim1b_WaveSpecific_Heatmap.html', 'rb') as f:
    telegram_send.send(conf='EcMetrics_Config_GeneralFlow.conf',
                       files=[f],
                       captions=["Simulated bias in VE estimates across study designs under 'wave-specific' parameter assumptions (T on vertical axis)"])
...
with open(path_method + 'VEMethod_Sim1b_WaveSpecific_Heatmap.xlsx', 'rb') as f:
    telegram_send.send(conf='EcMetrics_Config_GeneralFlow.conf',
                       files=[f],
                       captions=["Simulated bias in VE estimates across study designs under 'wave-specific' parameter assumptions (T on vertical axis)"])