...
# 0 --- Preliminaries
path_method = 'Output/'  # DOUBLE CHECK THIS
...
VE_consol_all_wide_grad = VE_consol_all_wide_reverse.style.background_gradient(cmap='coolwarm',
                                                                               vmin=lbound,
                                                                               vmax=ubound)
with open(path_method + 'VEMethod_Sim1b_Parallel_NoCI_Wide_Gradient.html', 'w') as f:
    f.write(VE_consol_all_wide_grad.render())
VE_consol_all_wide_grad.to_excel(path_method + 'VEMethod_Sim1b_Parallel_NoCI_Wide_Gradient.xlsx',
                                 sheet_name='Gradient',
                                 float_format='%.2f',
                                 na_rep='',
                                 index=True)
...
VE_consol_all_wide_rel_abs_grad = VE_consol_all_wide_reverse_rel_abs.style.background_gradient(
    cmap='Blues',
    vmin=0,
    vmax=10
)  # diff scheme and range since >=0
